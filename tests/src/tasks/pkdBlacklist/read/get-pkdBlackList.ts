...
export class GetPkdBlacklist extends RestTask<any>{

    name = 'get PKD blacklist'
    ...
    async process(holder: any): Promise<any> {
        this.response = await this.http.get(`/api/trade-credits/secured/pkdBlacklist/list`, holder.queryParams).disableTLSCerts();
