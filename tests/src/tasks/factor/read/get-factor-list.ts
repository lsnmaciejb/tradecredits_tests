...
export class GetFactorList extends RestTask<any>{

    name = 'get tcp Factor list'
    ...
    async process(holder: any): Promise<any> {
        this.response = await this.http.get(`/api/trade-credits/secured/factor/list`, holder.queryParams).disableTLSCerts();
